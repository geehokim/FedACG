...
class DriftTrainer(Trainer):
    ...
    def local_update(self, device, task_queue, result_queue):
        ...
        while True:
            ...
            client.setup(**setup_inputs)
            # Local Training
            local_model, local_loss_dict, drift = client.local_train(global_epoch=task['global_epoch'])
            result_queue.put((local_model, local_loss_dict, drift))
            if not self.args.multiprocessing:
                break
        ...
        for epoch in range(self.start_round, self.global_rounds):
            ...
            for i, client_idx in enumerate(selected_client_ids):
                ...
                if self.args.multiprocessing:
                    ...
                    local_state_dict, local_loss_dict, drift  = result_queue.get()
            ...
            if self.args.multiprocessing:
                for _ in range(len(selected_client_ids)):
                    # Retrieve results from the queue
                    result = result_queue.get()
                    local_state_dict, local_loss_dict, drift = result
