...
class BasicBlockWSFlex(nn.Module):
    expansion = 1

    def __init__(self, in_planes, planes, stride=1, use_bn_layer=False, Conv2d=WSConv2d):
        super(BasicBlockWSFlex, self).__init__()
        self.conv1 = Conv2d(
            in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn1 = nn.GroupNorm(min(16, planes//4), planes) if not use_bn_layer else nn.BatchNorm2d(planes) 
        self.conv2 = Conv2d(planes, planes, kernel_size=3,
                               stride=1, padding=1, bias=False)
        self.bn2 = nn.GroupNorm(min(16, planes//4), planes) if not use_bn_layer else nn.BatchNorm2d(planes) 
        ...
        if stride != 1 or in_planes != self.expansion*planes:
            self.downsample = nn.Sequential(
                ...
                nn.GroupNorm(min(16, planes//4), self.expansion*planes) if not use_bn_layer else nn.BatchNorm2d(self.expansion*planes) 
            )
...
class BottleneckWSFlex(nn.Module):
    expansion = 4
    
    def __init__(self, in_planes, planes, stride=1, use_bn_layer=False, Conv2d=WSConv2d):
        super(BottleneckWSFlex, self).__init__()
        ...
        self.bn3 = nn.GroupNorm(min(16, self.expansion*planes//4), self.expansion*planes) if not use_bn_layer else nn.BatchNorm2d(planes)
        ...
        if stride != 1 or in_planes != self.expansion*planes:
            self.downsample = nn.Sequential(
                Conv2d(in_planes, self.expansion*planes,
                          kernel_size=1, stride=stride, bias=False),
                nn.GroupNorm(min(16, self.expansion*planes//4), self.expansion*planes) if not use_bn_layer else nn.BatchNorm2d(planes)
            )
...
class ResNet_GNWSConv(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, l2_norm=False, use_pretrained=False, use_bn_layer=False,
        ...
        super(ResNet_GNWSConv, self).__init__()
        ...
        self.conv1 = WSConv2d(3, 64, kernel_size=conv1_kernel_size,
                               stride=1, padding=1, bias=False)
        self.bn1 = nn.GroupNorm(16, 64) if not use_bn_layer else nn.BatchNorm2d(64) 
...
class ResNet_GNWS(ResNet_GNWSConv):
...
class ResNet18_GNWS(ResNet_GNWS):
    
    def __init__(self, args: DictConfig, num_classes: int = 10, **kwargs):
        super().__init__(BasicBlockWSFlex, [2, 2, 2, 2], num_classes=num_classes, **kwargs
                        #  l2_norm=args.model.l2_norm,
                        #  use_pretrained=args.model.pretrained, use_bn_layer=args.model.use_bn_layer
                         )

@ENCODER_REGISTRY.register()
class ResNet34_GNWS(ResNet_GNWS):

    def __init__(self, args: DictConfig, num_classes: int = 10, **kwargs):
        super().__init__(BasicBlockWSFlex, [3, 4, 6, 3], num_classes=num_classes, **kwargs
                        #  l2_norm=args.model.l2_norm,
                        #  use_pretrained=args.model.pretrained, use_bn_layer=args.model.use_bn_layer
                         )
