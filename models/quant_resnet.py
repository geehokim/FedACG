...
def conv3x3(in_planes, out_planes, stride=1, groups=1, dilation=1, args=None):
    ...
    if args is not None and hasattr(args.model, 'keyword'):
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        # self.bn1 = norm_layer(2, inner_planes)
        self.bn1 = nn.GroupNorm(2, inner_planes)
    ...
    def forward(self, x):
        ...
        out = self.conv1(x)
        # print(f'BasicBlock forward {self.args.model.global_buffer}')
        out = self.bn1(out)
        out = self.relu1(out)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=100, zero_init_residual=False,
        ...
        self.bn1 = nn.GroupNorm(2, self.inplanes)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        
        # print(args)
        # print(args.model.keyword)
        
        if args is not None and hasattr(args.model, 'keyword'):
            if 'first-last' in args.model.keyword:
            ...
            if 'caffe-pooling' in args.model.keyword:
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            force_fp = False
            if self.args is not None and hasattr(self.args.model, 'keyword'):
                if 'real_skip' in self.args.model.keyword:
                    force_fp = True
    ...
    def forward(self, x):
        ...
        if hasattr(self.args.model, 'global_buffer'):
            quant_loss = self.args.model.global_buffer.get('quant_loss', 0)
            if 'quant_loss' in self.args.model.global_buffer:
                self.args.model.global_buffer['quant_loss'] = 0
...
def _resnet(arch, block, layers, pretrained, progress, args):

    model = ResNet(block, layers, args = args)
    if pretrained:
...
def resnet18(pretrained=False, progress=True, args = None):
...
class ResNetWrapper(nn.Module):
    def __init__(self, args, num_classes=100, pretrained=False, progress=True, **kwargs):
...
def quant_resnet18(args, num_classes=100, **kwargs):
