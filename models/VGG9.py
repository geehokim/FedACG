...
class VGG9_base(nn.Module):
    ...
    def __init__(self, args, num_classes=10 ,**kwargs):
        ...
        self.layer1 = nn.Sequential(nn.Conv2d(3, 32, kernel_size=3, padding=1, bias=False) ,nn.GroupNorm(2, 32), nn.ReLU())
        self.layer2 = nn.Sequential(nn.Conv2d(32, 64, kernel_size=3, padding=1, bias=False),nn.GroupNorm(2, 64), nn.ReLU(), nn.MaxPool2d(kernel_size=2, stride=2))
        self.layer3 = nn.Sequential(nn.Conv2d(64, 128, kernel_size=3, padding=1, bias=False) ,nn.GroupNorm(2, 128), nn.ReLU())
        self.layer4 = nn.Sequential(nn.Conv2d(128, 128, kernel_size=3, padding=1, bias=False) ,nn.GroupNorm(2, 128), nn.ReLU(), nn.MaxPool2d(kernel_size=2, stride=2) )
        self.layer5 = nn.Sequential(nn.Conv2d(128, 256, kernel_size=3, padding=1, bias=False) ,nn.GroupNorm(2, 256), nn.ReLU() )
        self.layer6 = nn.Sequential(nn.Conv2d(256, 256, kernel_size=3, padding=1, bias=False) ,nn.GroupNorm(2, 256), nn.ReLU() , nn.MaxPool2d(kernel_size=2, stride=2) )
